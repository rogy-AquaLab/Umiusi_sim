...
- **`v_cmd` の大きさは `vel_cmd_cap_frac * VEL_PER_CAP * max_duty` を超えてはならない**
  (cap 0.25 で **約 0.136 m/s**、cap 0.4 で約 0.218 m/s)。定数は
  `umiusi_pose_env.py` の `VEL_PER_CAP = 0.68` と config の `vel_cmd_cap_frac: 0.8`。

  **`vel_cmd_max: 0.4` は上限ではない** — cap 0.25 での物理的到達可能速度は
  `0.68 * 0.25 ≈ 0.17 m/s` で、0.4 は既に物理の外である。学習が到達可能な指令だけを
  サンプルしているのは `vel_cmd_cap_frac` のためで、この仕組みを迂回して大きい指令を
  渡すと方策は分布外に出る。**指令を大きくするほど遅くなる** (av_mode13 実測、公称条件、
  指令の大きさを固定して 8 エピソード/点):

  | \|v_cmd\| | ori [rad] | null(pw) | 実速度 along [m/s] |
  |---:|---:|---:|---:|
  | 0.20 | 0.231 | 6.7% | 0.146 |
  | 0.40 | 0.354 | 9.6% | 0.161 |
  | 1.00 | **0.589** | **14.0%** | **0.114** |

  1.0 は UI (`gamepadPublisher.ts`) が `velocity.x` に送る最大値で、**姿勢が 34° まで崩れ、
  かつ 0.4 のときより遅くなる**。UI かノードのどちらかで上式にクランプすること。cap は
  運用中に変わるので、cap を読めるノード側でのクランプが確実。

  **`VEL_PER_CAP` は推力/抗力の較正で動く**(定数の定義箇所にも明記)。較正したら
  この値も測り直し、クランプする側が参照している値も更新すること。
