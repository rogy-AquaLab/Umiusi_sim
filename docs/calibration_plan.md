...
**`max_duty` プロトコル: 0.2 → 0.4。** 0.2 は初回の安全確認用の絞りであり、以降の較正・運用は 0.4 で行う。
**深度 / 鉛直モードの試験は 0.4 必須**（推力がプロペラ則のため 0.2 では実効推力が 1/4 になり、そもそも降下できない）。
...
## 3. スラスタ推力マップ（秤、ドライ〜バケツ） ← **最優先**

`thrust_per_cmd` と `thrust_curve_exp` は**両方とも未較正で、しかも互いに交絡**している
（水中データが `|u| <= 0.2` しか無く、この範囲では「小さい K で緩い指数」と「大きい K で
急な指数」が区別できない）。sim の残差より この不確かさのほうが桁で大きいので、**次の実験で
最も効くのはここ**。

**手順**: スラスタを秤に固定し（水没させて）、**サーボは 0°（推力軸を秤の測定方向に合わせる）**
で固定。各 duty で推力 [N] を記録。1 基で良い（`thrust_unit_frac` の DR が個体差を吸収）。

| 段 | duty | 位置づけ |
|---|---|---|
| **必須** | ±0.2 / ±0.3 / ±0.4 / ±0.5 | **運用域**（`max_duty` は 0.25、鉛直試験でも 0.4）。`F = K·\|u\|^e` の **K と e はこのレンジのデータだけで両方決まる** |
| ベストエフォート | ±0.7 / ±1.0 | `K` を外挿ではなく実測で置ける。後で `max_duty` を上げる判断をするときに効く |

時間が押したら後半 2 点を落として良い（主目的は落ちない）。逆に**必須の 4 点は運用域そのもの
なので落とさない**。

> **`/cmd/direct` の duty クランプに注意**: control の PR #308
> (`fix/actuator-limits-on-direct-cmd`) が入ると `/cmd/direct` にも `max_duty` が掛かる。
> `params/controllers.yaml` の値は **0.5** なので、**#308 込みの環境では ±0.7 / ±1.0 が
> 取れない**（0.5 に張り付く）。必須の 4 点は 0.5 以下なので影響しない。高 duty 側も取るなら、
> 較正 run のあいだだけ `max_duty` を 1.0 に上げること。

**反映**: `configs/umiusi.yaml` の `thrust_per_cmd`（現在 30 N は出典不明）と
`thrust_curve_exp`（現在 2.0）。正負非対称が出たら `thruster.py` に多項式マップを足す。

**較正したら必ず**: ① 配備済み方策を **再 export**する（`action_contract` が config の値を
写して実機がそこからプラントを再現するため、config だけ直しても実機には伝わらない）
② `umiusi_pose_env.py` の `VEL_PER_CAP`（開ループ終端速度／cap）を測り直す — 推力が変われば
到達可能速度も変わり、`v_cmd` の上限式（`docs/rl.md` 運用プロトコル）が狂う。
...
> ## ⚠ このフィットは汚染されている（2026-09-06 判明）— 上の 2 値は暫定
>
> `bag_replay.py` は bag に記録された **50 Hz の指令**を sim に食わせている。しかし当時の
> 実機 CAN スタックがハードウェアに届けていたのは **6.25 Hz（160 ms 周期）** だった
> （16 スロット round-robin × `on_write` 1 回/周期。control の `82752a8` が
> `tools/can_rate.py` で実測、`FRAMES_PER_UPDATE` 導入前）。
>
> **つまり sim は、実機が受け取っていない指令の 7/8 を余分に効かせてフィットしていた。**
> しかも当時の esc-action 方策は毎ステップ符号反転するバンバンだったので、8:1 の間引きは
> 実効推力を大きく変える。**「実推力が線形予測の ~1/10」は、推力曲線ではなく指令の
> 取りこぼしで説明できる可能性がある**（1/8 と 1/10 が近いのは示唆的だが偶然かもしれない）。
>
> **これは時系列ではなく、Pi に載っているブランチ次第で決まる**（`82752a8` を含むかどうか）:
>
> | control の状態 | 指令あたり到達レート | 合計 TX | 50 Hz 指令に対して |
> |---|---|---|---|
> | `82752a8` 以前 = **8/21 の bag / 2026-09-06 時点の Pi**（実測） | **6.25 Hz** | 50 f/s | **8:1** |
> | `82752a8`（早期 return あり、実測） | 12.1 Hz | 96.8 f/s | 4:1 |
> | `f7234df`（`continue` 修正、未実測） | 50 Hz | 400 f/s | 1:1 |
>
> **Pi に何を焼いたかを run ごとに記録すること。** `candump -l can0` を並走させれば
> 事後に確定できる（ID ごとのフレーム数を秒で割るだけ）。
>
> ### この汚染が較正実験のどれに効くか
>
> **一定指令の実験は影響を受けない**。指令が変化しないなら、6.25 Hz の ZOH は 50 Hz と
> 同じ値を出し続ける。効くのは**指令が速く変わるとき**だけで、8/21 の bag が壊滅的だったのは
> 当時の方策が毎ステップ符号反転していたからである。
>
> | | 指令の性質 | 影響 |
> |---|---|---|
> | §1 スラスタ ID | 2 秒間の一定指令 | **無し** |
> | §2 サーボ応答 | ステップ | **有り**。遅れではなく**振動**で効く: VESC 側のサーボ指令タイムアウトが 160 ms より短いと「受信 → タイムアウト → 中立」を繰り返してサーボが振動する（`82752a8` の実機観察: ROS 経由で角度を保持すると振動、VESC から直接動かすと振動しない）。6.25 Hz = 160 ms 周期はまさにこの条件。**サーボ応答は CAN を 50 Hz に戻してから測ること** |
> | §3 推力マップ | 秤・一定指令 | **無し**（指令経路そのものをバイパスする） |
> | §4 浮力と復元 | 無指令 | **無し** |
> | §5 並進抗力 | 一定 duty | **無し** |
> | §6 回転抗力 | 一定トルク | **無し** |
> | §7 付加質量 | ステップ **＋過渡の時間を測る** | **有り**。ステップが最大 160 ms 遅れて着地するので、加速度過渡から付加質量をフィットする際に効く。**指令のタイムスタンプではなく、観測された応答の立ち上がりを基準に測ること**（それで大きく緩和できる） |
>
> **どちらの値も直接測定で置き換えるまで信用しないこと。** 置き換え先は既に計画にある:
> `thrust_curve_exp` / `thrust_per_cmd` → **§3（秤。指令経路を丸ごとバイパスする）**、
> `buoyancy_offset_above_com` → **§4（静置。そもそも指令を出さない）**。
> **§3 / §4 の前に、この 2 値を根拠にした議論をしないこと。**
>
> 再フィットで交絡の大きさを見たい場合は、指令列を **6.25 Hz（8:1）** に間引いて回す。
