...
class UmiusiSimulator:
    def __init__(self, model_path=_DEFAULT_MODEL, config_path=_DEFAULT_CONFIG):
        ...
        # Higher-fidelity hydro, all config-gated (absent key -> off = plain diagonal drag):
        #   lift        : force perpendicular to the body-frame flow, grows with angle of attack
        #   cop_offset  : drag applied at a center-of-pressure OFFSET from the CoM, so a broadside
        #                 translation induces a turning/righting (Munk-like) moment
        #   coupling    : off-diagonal (sway->yaw, heave->pitch) damping moments
        ...
        # 1.0 = no cap; training uncapped makes the policy saturate against the robot's clamp.
        self.max_duty = float(t.get("max_duty", 1.0))
        # action[k] drives the unit NAMED action_order[k] — the autonomy-side POSITIONS contract,
        # NOT id order. A config without per-unit names falls back to id order.
        ...
        # camera_degrade: render_camera() applies depth-buffer-based underwater degradation (colour
        # attenuation + haze, from perception.underwater_sim) so perception sees murky footage.
        # water_params fixes the water condition for the run (set at reset); None -> murky default.
    ...
    def _apply_external_forces(self):
        ...
        # Translational drag FORCE at the center of pressure (CoM + cop_offset): the moment arm
        # turns a broadside translation into a righting/turning moment. The angular damping +
        # coupling moment (w[3:]) is a free vector, so it goes in as a pure moment.
